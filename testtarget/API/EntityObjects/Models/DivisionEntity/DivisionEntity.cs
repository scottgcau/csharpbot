...
namespace APITests.EntityObjects.Models
{
	public class DivisionEntity : BaseEntity
	{
		...
		public DivisionEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseAttributes();
			InitialiseReferences();
		...
		private static string GetInvalidSeasonId(string validator)
		{
			switch (validator)
			{
				case "Required":
					return "";
				default:
					throw new Exception($"Cannot find validator {validator} for attribute Divisions");
			}
		}
		...
		// TODO needs some warning if trying to get an invalid entity, and the entity
		// attributes don't actually have any validators to violate.
		public static DivisionEntity GetEntity(bool isValid, string fixedValue = null)
		{
		...
		public static DivisionEntity GetInvalidEntity()
		{
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static DivisionEntity GetValidEntity(string fixedStrValue = null)
		{
			var divisionEntity = new DivisionEntity
			{
		...
		{
			return SaveToDB<Sportstats.Models.DivisionEntity>(DivisionEntityDto.Convert(this));
		}
	}
}
