...
namespace APITests.EntityObjects.Models
{
	public class LeagueEntity : BaseEntity
	{
		...
		public LeagueEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseAttributes();
			InitialiseReferences();
		...
		private static string GetInvalidSportId(string validator)
		{
			switch (validator)
			{
				case "Required":
					return "";
				default:
					throw new Exception($"Cannot find validator {validator} for attribute Leagues");
			}
		}
		...
		public override void SetReferences (Dictionary<string, ICollection<Guid>> entityReferences)
		{
			foreach (var (key, guidCollection) in entityReferences)
			{
				...
				{
					case "SportId":
						ReferenceIdDictionary.Add("SportId", guidCollection.FirstOrDefault());
		...
		// TODO needs some warning if trying to get an invalid entity, and the entity
		// attributes don't actually have any validators to violate.
		public static LeagueEntity GetEntity(bool isValid, string fixedValue = null)
		{
		...
		public static LeagueEntity GetInvalidEntity()
		{
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static LeagueEntity GetValidEntity(string fixedStrValue = null)
		{
			var leagueEntity = new LeagueEntity
			{
				Name = Guid.NewGuid().ToString(),
		...
		{
			return SaveToDB<Sportstats.Models.LeagueEntity>(LeagueEntityDto.Convert(this));
		}
	}
}
