...
{
	public class SportentityFormTile : BaseEntity	{

		// The tile that the form is contained in
		public String Tile { get; set; }

		public SportentityFormTile()
		{
			EntityName = "SportentityFormTile";

			InitialiseAttributes();
			InitialiseReferences();
		}

		public SportentityFormTile(ConfigureOptions option)
		{
			Configure(option);
			InitialiseAttributes();
			InitialiseReferences();
		...
		public override string GetInvalidAttribute(string attribute, string validator)
		{
			switch (attribute)
			{
				...
				case "FormId":
					return GetInvalidSportentityId(validator);
				default:
		...
		private static string GetInvalidSportentityId(string validator)
		{
			switch (validator)
			{
				case "Required":
					return null;
				default:
					throw new Exception($"Cannot find validator {validator} for attribute Sportentity");
			}
		}
			...
			return new List<(List<string> expectedError, JsonObject jsonObject)>
			{
				...
				(
					...
					new JsonObject
					{
						["id"] = Id,
						["tile"] = Tile.ToString(),
						["formId"] = GetInvalidSportentityId("Required"), // invalid stationId
					}
				),
			};
		}
		...
		// TODO needs some warning if trying to get an invalid entity, and the entity
		// attributes don't actually have any validators to violate.
		public static SportentityFormTile GetEntity(bool isValid, string fixedValue = null)
		{
		...
		public static SportentityFormTile GetInvalidEntity()
		{
		...
		{
			FormId = new Sportentity(ConfigureOptions.CREATE_ATTRIBUTES_AND_REFERENCES).Save();
		}
		...
		{
			Tile = BaseChoice.GetValidString();
		}

		/// <summary>
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static SportentityFormTile GetValidEntity(string fixedStrValue = null)
		{
			return new SportentityFormTile
			{
				Tile = (!string.IsNullOrWhiteSpace(fixedStrValue) && fixedStrValue.Length > 0 && fixedStrValue.Length <= 255) ? fixedStrValue : BaseChoice.GetValidString(),
			};
		}

		public override Guid Save()
		{
			return SaveToDB<Sportstats.Models.SportentityFormTile>(SportentityFormTileDto.Convert(this));
		}
	}
}
