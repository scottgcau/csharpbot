...
try {
	...
	do {
		Start-Sleep 1;
		$serverStatus = docker inspect --format='{{.State.Health.Status}}' sportstats_server_1
		...
		$clientStatus = docker inspect --format='{{.State.Health.Status}}' sportstats_client_1;
	...
	} Until((docker-compose --project-name "sportstats" ps | Select-String 'Up \(healthy\)' | Measure-Object -Line).Lines -gt 1)
