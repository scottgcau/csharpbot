...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the Roster Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("RosterEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class RosterEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		public RosterEntityFormVersion()
		{
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(RosterEntity))]
		public RosterEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.RosterEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(RosterEntity))]
		public RosterEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.RosterSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(RosterSubmissionEntity))]
		public ICollection<RosterSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.RosterEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<RosterEntityFormVersion>().ToList();
