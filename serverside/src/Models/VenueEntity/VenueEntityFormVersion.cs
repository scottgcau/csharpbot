...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the Venue Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("VenueEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class VenueEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(VenueEntity))]
		public VenueEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.VenueEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(VenueEntity))]
		public VenueEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.VenueSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(VenueSubmissionEntity))]
		public ICollection<VenueSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.VenueEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<VenueEntityFormVersion>().ToList();
