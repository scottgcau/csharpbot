...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the Person Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("PersonEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class PersonEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		public PersonEntityFormVersion()
		{
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(PersonEntity))]
		public PersonEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.PersonEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(PersonEntity))]
		public PersonEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.PersonSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(PersonSubmissionEntity))]
		public ICollection<PersonSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.PersonEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<PersonEntityFormVersion>().ToList();
