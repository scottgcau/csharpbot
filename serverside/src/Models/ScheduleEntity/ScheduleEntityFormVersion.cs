...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the Schedule Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("ScheduleEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class ScheduleEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		public ScheduleEntityFormVersion()
		{
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(ScheduleEntity))]
		public ScheduleEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.ScheduleEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(ScheduleEntity))]
		public ScheduleEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.ScheduleSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(ScheduleSubmissionEntity))]
		public ICollection<ScheduleSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.ScheduleEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<ScheduleEntityFormVersion>().ToList();
