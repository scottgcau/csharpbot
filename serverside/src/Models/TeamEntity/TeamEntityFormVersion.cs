...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the Team Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("TeamEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class TeamEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(TeamEntity))]
		public TeamEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.TeamEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(TeamEntity))]
		public TeamEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.TeamSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(TeamSubmissionEntity))]
		public ICollection<TeamSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.TeamEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<TeamEntityFormVersion>().ToList();
