...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the RosterAssignment Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("RosterassignmentEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class RosterassignmentEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		public RosterassignmentEntityFormVersion()
		{
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(RosterassignmentEntity))]
		public RosterassignmentEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.RosterassignmentEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(RosterassignmentEntity))]
		public RosterassignmentEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.RosterassignmentSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(RosterassignmentSubmissionEntity))]
		public ICollection<RosterassignmentSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.RosterassignmentEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<RosterassignmentEntityFormVersion>().ToList();
