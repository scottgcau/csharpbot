...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the Game Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("GameEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class GameEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(GameEntity))]
		public GameEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.GameEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(GameEntity))]
		public GameEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.GameSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(GameSubmissionEntity))]
		public ICollection<GameSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.GameEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<GameEntityFormVersion>().ToList();
