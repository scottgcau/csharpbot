...
namespace Sportstats.Models {
	...
	public class GamerefereeEntity : IOwnerAbstractModel	{
		[Key]
		...
		[EntityForeignKey("Game", "Gamerefereess", false, typeof(GameEntity))]
		...
		[EntityForeignKey("Person", "Gamereferee", false, typeof(PersonEntity))]
		...
		{
			var modelList = models.Cast<GamerefereeEntity>().ToList();
			var ids = modelList.Select(t => t.Id).ToList();

			switch (reference)
			{
				case "Person":
					var personIds = modelList.Select(x => x.Person.Id).ToList();
					var oldperson = await dbContext.PersonEntity
						.Where(m => m.GamerefereeId.HasValue && ids.Contains(m.GamerefereeId.Value))
						.Where(m => !personIds.Contains(m.Id))
						.ToListAsync(cancellation);

					foreach (var person in oldperson)
					{
						person.GamerefereeId = null;
					}

					dbContext.PersonEntity.UpdateRange(oldperson);
					return oldperson.Count;
