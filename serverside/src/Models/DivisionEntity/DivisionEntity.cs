...
namespace Sportstats.Models {
	...
	public class DivisionEntity : IOwnerAbstractModel	{
		[Key]
		...
		[EntityForeignKey("Teamss", "Division", false, typeof(TeamEntity))]
		...
		[EntityForeignKey("Season", "Divisionss", false, typeof(SeasonEntity))]
		...
		{
			var modelList = models.Cast<DivisionEntity>().ToList();
			...
			switch (reference)
			{
				case "Teamss":
					...
					var oldteams = await dbContext.TeamEntity
						.Where(m => m.DivisionId.HasValue && ids.Contains(m.DivisionId.Value))
						.Where(m => !teamsIds.Contains(m.Id))
						.ToListAsync(cancellation);

					foreach (var teams in oldteams)
					{
						teams.DivisionId = null;
					}
