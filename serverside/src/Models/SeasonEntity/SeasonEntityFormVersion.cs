...
namespace Sportstats.Models {
	/// <summary>
	/// The form versions for the Season Entity form behaviour
	/// </summary>
	// % protected region % [Configure entity attributes here] off begin
	[Table("SeasonEntityFormVersion")]
	// % protected region % [Configure entity attributes here] end
	public class SeasonEntityFormVersion : IOwnerAbstractModel	{
		[Key]
		...
		public SeasonEntityFormVersion()
		{
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(SeasonEntity))]
		public SeasonEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Sportstats.Models.SeasonEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(SeasonEntity))]
		public SeasonEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Sportstats.Models.SeasonSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(SeasonSubmissionEntity))]
		public ICollection<SeasonSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			if (PublishVersion)
			{
				var formModel = dbContext.SeasonEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		{
			var modelList = models.Cast<SeasonEntityFormVersion>().ToList();
